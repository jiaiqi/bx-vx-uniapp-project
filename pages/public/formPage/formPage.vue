...
export default {
	...
	onLoad(option) {
		...
		if (option.params) {
			if(typeof option.params==='string'&&(option.params.indexOf('%7B')||option.params.indexOf('%3A'))!==-1){
	...
	methods: {
		toChildServiceList(e) {
			...
			if (e.foreign_key && e.foreign_key.more_config && formData[e.foreign_key.referenced_column_name]) {
				...
				uni.navigateTo({
					url: '/pages/public/list/list?pageType=list&serviceName=' + e.service_name + '&cond=' + JSON.stringify(condition)+ '&showNavBack=' + true
				});
			}
		},
		toUpdatePages() {
