<template>
  <view>
    ...
    <bx-list ref="bxList" :appno="appName" :serviceName="serviceName" :condition="condition"
      ...
      @click-list-item="clickItem" @list-change="listChange" @clickFootBtn="clickFootBtn" @loadEnd="loadEnd"></bx-list>
  ...
  export default {
      ...
      if (query.cond) {
        try {
          ...
          if (Array.isArray(cond)) {
            cond.forEach(item => {
              if ((item.colName === 'create_user' || item.colName === 'openid' || item.colName === 'glry') && item
                .value === 'user_no') {
    ...
    methods: {
        ...
        if (this.pageType === 'proc') {
          this.publicButton.map(item => {
            if (item.button_type === 'add' || item.button_type === 'apply') {
              uni.navigateTo({
                url: '/pages/public/proc/apply/apply?serviceName=' + item.service_name + '&cond=' +
          ...
          this.publicButton.map(item => {
            if (item.button_type === 'add') {
              ...
              uni.navigateTo({
                url: '../formPage/formPage?serviceName=' + item.service_name + '&type=add&cond=' +
                  decodeURIComponent(JSON.stringify(this.condition))
              });
        ...
        if (buttonInfo.operate_params && typeof buttonInfo.operate_params === 'string') {
          try {
            ...
            if (buttonInfo.operate_type === '操作' && buttonInfo.operate_mode === '静默操作') {
              ...
              let url = this.getServiceUrl(buttonInfo.application ? buttonInfo.application : app, buttonInfo
                .operate_service, buttonInfo.servcie_type);
      ...
      async getListV2() {
        ...
        let self = this;
        let colVs = await this.getServiceV2(this.serviceName, 'list', this.pageType === 'proc' ? 'proclist' : 'list',
          app);
              ...
              case 'customize':
                if (item.application === 'zhxq' && item.button_name === '住户录入') {
                  if (self.queryOption && self.queryOption.hasOwnProperty('showAdd')) {
                    self.showAdd = self.queryOption.showAdd === 'false' ? false : self.queryOption.showAdd ==
                      'true' ? true : self.queryOption.showAdd;
                  } else {
                    self.showAdd = true;
                  }
                  return item;
    ...
    onShareAppMessage(res) {
      if (res.target.dataset.info.button_type === 'applyProc' && res.target.dataset.info.button_name === '邀请' && res
        .target.dataset.procno) {
        return {
          title: '绑定住户邀请',
        ...
        return {
          title: '绑定住户邀请',
          path: '/pages/specific/shareBind/shareBind?dataid=' + res.target.dataset.id +
            '&serviceName=srvzhxq_member_update'
        };
      } else {
        return {
          title: '分享测试',
