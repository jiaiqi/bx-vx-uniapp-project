		...
		<view v-if="disabled">
			<view class="" v-for="(file,index) in filesListDataRun" :key="index">
				...
				<view class="thumb-view flex">
					...
					<button class="dowloadBtn blue" @click="file.icon === 'img'  ? previewImage(file) : dowloadFile(file)">{{file.icon === 'img'  ? '预览' : '下载'}}</button>
				</view>
			</view>
		</view>
		<view v-else>
			...
			<button type="primary" plain="true" :loading="loaded" v-if="filesListDataRun.length < maxNumber" @click="selectImage">上传</button>
		</view>
	</view>
</template>
		...
		computed:{
			filesListDataRun:function(){ 
				...
				defaultVal = defaultVal.map((item) =>{
					if(item.type.indexOf('doc') !== -1 || item.type.indexOf('pdf') !== -1 || item.type.indexOf('xls') !== -1){
						item.icon = 'doc'
					}else if(item.type.indexOf('png') !== -1 || item.type.indexOf('jpg') !== -1 || item.type.indexOf('jpeg') !== -1|| item.type.indexOf('gif') !== -1){
						item.icon = 'img'
					}else if(item.type.indexOf('mp4') !== -1 || item.type.indexOf('avi') !== -1 || item.type.indexOf('mov') !== -1|| item.type.indexOf('gif') !== -1){
						item.icon = 'video'
					}else {
						item.icon = 'file'
					}
		...
		methods:{
			...
			deleteFile: function(e,index){
				...
				if(deletedImagePath.indexOf('&') !== -1){
					fileUrl = deletedImagePath.substring(deletedImagePath.lastIndexOf('filePath=') + 9,deletedImagePath.indexOf('&'))
				}
				if(self.deleteApi){
