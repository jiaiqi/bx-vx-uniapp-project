  ...
  <view>
    <view class="cu-form-group itemwrap" v-if="fieldData.display" :class="{ 'form-detail': pageFormType === 'detail' }">
      <view class="title" :class="!valid.valid ? 'valid_error' : ''">
        ...
        <text :class="isDisabled  ? 'text-gray' : 'text-blue'">{{ fieldData.label }}:</text>
        <text v-show="!valid.valid">({{ valid.msg }})</text>
        <view class="info text-orange" v-if="valid.info&&valid.info!==true">
          {{valid.info}}
        </view>
      </view>
      <!-- 图片描述 -->
      <block class="img-explain" v-if="fieldData.option_img_explain">
        ...
        <u-image width="100%" height="300rpx" :src="getOptionImgExplain(fieldData.option_img_explain)"
          v-if="showOption_img"></u-image>
      </block>
      <view v-if="pageFormType === 'detail'" class="detail-text" :class="{'row-wrap':fieldData.isMulti}">
        <text class=""
          v-if="pageFormType === 'detail' && fieldData.type !== 'images' && fieldData.type !== 'snote' && fieldData.type !== 'Note' && fieldData.type !== 'file' && fieldData.col_type !== 'UserList'">
          {{ fieldData.valueForDisplay?fieldData.valueForDisplay: dictShowValue ? dictShowValue : treeSelectorShowValue ? treeSelectorShowValue : fieldData.value }}
        </text>
        ...
        <view class="" v-html="fieldData.value"
          v-if="pageFormType === 'detail' && (fieldData.type === 'snote' || fieldData.type === 'Note')">
        </view>
        <view class="" v-else-if="fieldData.type === 'file'">
          <bxUploader :onAdd="false" :onDelete="false" :disabled="true" :formData="formDataRun"
            :serverHeader="reqHeader" :value="imagesUrl" :uploadApi="upLoadUrl" :deleteApi="deleteFileUrl"
            @on-add-change="addFileChange" @on-delete-change="deleteFileChange"></bxUploader>
        </view>
        <view class="" v-else-if="pageFormType === 'detail' && fieldData.type === 'images'">
          <image v-if="fieldData.type === 'images'" v-for="(item, index) in imagesUrl" :key="index"
            style="padding: 5upx;" class="cu-avatar radius lg" @tap="showModal(index, 'Image')" data-target="Image"
            :src="item"></image>
        </view>
      </view>

      <view class="form-content" :class="{
        ...
        <u-radio-group v-model="fieldData.value" @change="radioChange" v-if="fieldData.type === 'radio'"
          :class="!valid.valid ? 'valid_error' : ''">
          <u-radio class="radio" color="#0bc99d" :disabled="isDisabled ? isDisabled : false"
            v-for="(itema, index) in optionList" :name="itema" v-model="fieldData.value">
        ...
        <!--<radio-group @change="radioChange" v-if="fieldData.type === 'radio'" :class="!valid.valid ? 'valid_error' : ''">
					<radio
						...
						v-for="(itema, index) in optionList"
						class="radio"
        ...
        <u-radio-group v-model="fieldData.value" @change="radioChange" v-if="fieldData.type === 'radioFk'"
          :class="!valid.valid ? 'valid_error' : ''">
          <u-radio class="radio" color="#2979ff" :disabled="isDisabled ? isDisabled : false"
            v-for="(itema, index) in optionList" :name="itema.value" v-model="fieldData.value">
            <view class="radio-label">{{ itema.label ? itema.label : itema.value }}</view>
            <u-image width="100%" height="300rpx" v-if="itema.option_img_explain"
              :src="getOptionImgExplain(itema.option_img_explain)" mode="aspectFit"></u-image>
          </u-radio>
        </u-radio-group>
        ...
        <checkbox-group name="checkbox-group" class="checkbox-group" v-else-if="fieldData.type === 'checkbox'"
          :class="!valid.valid ? 'valid_error' : ''">
          <label v-for="(item, index) in optionList" :key="index" class="checkbox" @click="radioChange(item)">
            <checkbox color="#2979ff" :value="item" :disabled="isDisabled ? isDisabled : false"
              :checked="fieldData.value.indexOf(item) !== -1" />
            <text style="flex: 1;" class="text">{{ item }}</text>
          </label>
        </checkbox-group>
        <checkbox-group name="checkbox-group" class="checkbox-group" v-else-if="fieldData.type === 'checkboxFk'"
          :class="!valid.valid ? 'valid_error' : ''">
          <label v-for="(item, index) in optionList" :key="index" class="checkbox" @click="radioChange(item, index)">
            <checkbox color="#2979ff" class="blue" :class="isChecked(item.value) ? 'checked' : ''"
              :checked="isChecked(item.value) ? true : false" :value="item.value"></checkbox>
            <text style="flex: 1;" class="text">{{ item.label }}</text>
          </label>
        </checkbox-group>
        <view v-else-if="fieldData.type === 'images'">
          <robby-image-upload :value="imagesUrl" :enable-del="isDisabled ? !isDisabled : true"
            :enable-add="isDisabled ? !isDisabled : true" :server-url="upLoadUrl" @delete="deleteImage"
            @add="getImageInfo" :form-data="formData" :header="reqHeader" :showUploadProgress="true"
            :server-url-delete-image="deleteFileUrl" :limit="fieldData.fileNum"></robby-image-upload>
        </view>
        <view class="" v-else-if="fieldData.type === 'file'">
          <bxUploader :onAdd="false" :onDelete="false" :disabled="isDisabled" :formData="formDataRun"
            :serverHeader="reqHeader" :value="imagesUrl" :uploadApi="upLoadUrl" :deleteApi="deleteFileUrl"
            @on-add-change="addFileChange" @on-delete-change="deleteFileChange"></bxUploader>
        </view>
        <textarea style="min-height: 60px;width: 100%;"
          :maxlength="fieldData.item_type_attr && fieldData.item_type_attr.max_len ? fieldData.item_type_attr.max_len : 100"
          @blur="onInputBlur" @focus="onInputfocus" auto-height v-model="fieldData.value" @input="onInputChange"
          :disabled="isDisabled ? isDisabled : false" :class="!valid.valid ? 'valid_error' : ''"
          v-else-if="fieldData.type === 'textarea' && showTextarea" :placeholder="'输入' + fieldData.label"></textarea>
        <input @click="!isDisabled?showRichText = true:''" type="text"
          v-if="(fieldData.type === 'snote' || fieldData.type === 'Note')" :placeholder="'点击编辑' + fieldData.label"
          :value="html2text(fieldData.value)" :class="!valid.valid ? 'valid_error' : ''" name="input"
          :disabled="true" />
        <picker class="pickers" @change="PickerChange($event, fieldData)" :value="index" :range="picker"
          v-if="fieldData.type === 'poupchange'">
          <input type="text" :placeholder="'点击编辑' + fieldData.label" :value="picker[index]"
            :class="!valid.valid ? 'valid_error' : ''" name="input" :disabled="true" />
        </picker>
        <view class="content padding-0"
          ...
          <view class="cu-list menu card-menu ">
            <view class="cu-item" v-for="(item, index) in optionsDatasRun" :key="index">
              <view class="content solid">
                <text class="cuIcon-tagfill text-blue  margin-right-xs"
                  :class="{ 'text-green': '' }">{{ index + 1 }}</text>
                <input class="input-sm" type="text" disabled :placeholder="'点击右侧按钮编辑'"
              ...
              <view class="buttons">
                ...
                <view v-if="item.type !== 'draft' && item['model']['option_value']" class="cu-btn  bg-blue light"
                  style="height:2.4em;min-height: 1.6em;line-height: 1.6em;" @click="addListOptions(index, 'update')">
                  编辑
                </view>
                <view v-if="item.type === 'draft'" class="cu-btn  bg-blue light"
                  style="height:2.4em;min-height: 1.6em;line-height: 1.6em;" @click="addListOptions(index, 'add')">
                  添加
                </view>
                <view v-if="item.type !== 'draft'" class="cu-btn  bg-orange light"
                  style="height:2.4em;min-height: 1.6em;line-height: 1.6em;" @click="deleteListOptions(index)">
                  删除
                </view>
              </view>
            </view>
        ...
        <input @blur="onInputBlur" type="number" :placeholder="'输入' + fieldData.label" @input="onInputChange"
          ...
          v-model.number="fieldData.value" :class="!valid.valid ? 'valid_error' : ''" name="input"
          :disabled="isDisabled ? isDisabled : false" v-else-if="fieldData.type === 'number'" />
        <input @blur="onInputBlur" type="digit" :placeholder="'输入' + fieldData.label" @input="onInputChange"
          :disabled="isDisabled ? isDisabled : false"
          :max="fieldData.item_type_attr && fieldData.item_type_attr.max ? fieldData.item_type_attr.max : 999999"
          :min="fieldData.item_type_attr && fieldData.item_type_attr.min ? fieldData.item_type_attr.min : 0"
          v-model.number="fieldData.value" :class="!valid.valid ? 'valid_error' : ''" name="input"
        ...
        <view v-else-if="fieldData.type === 'treeSelector'" @click="openTreeSelector">
          ...
          <!-- 				<input :placeholder="'点击选择' + fieldData.label"
						:value="fieldData.valueForDisplay?fieldData.valueForDisplay:treeSelectorShowValue" disabled
						:class="!valid.valid ? 'valid_error' : ''" name="input" /> -->
        </view>
        <view v-else-if="fieldData.type === 'cascader'" @click="openCascader">
          <input :placeholder="'点击选择' + fieldData.label" v-model="fieldData.value" disabled
            :class="!valid.valid ? 'valid_error' : ''" name="input" />
        </view>
        <view class="item-group flex align-center" style="" v-else-if="fieldData.type === 'input'">
          <input @blur="onInputBlur"
            :maxlength="fieldData.item_type_attr && fieldData.item_type_attr.max_len ? fieldData.item_type_attr.max_len : 100"
            :disabled="isDisabled ? isDisabled : false" :placeholder="'输入' + fieldData.label" v-model="fieldData.value"
            @input="onInputChange" :class="!valid.valid ? 'valid_error' : ''" name="input" type="text" />
        </view>
        <view class="item-group flex align-center" style="" v-else-if="fieldData.type.indexOf('undefined')!==-1">
          <input @blur="onInputBlur" :disabled="isDisabled ? isDisabled : false" :placeholder="'输入' + fieldData.label"
            v-model="fieldData.value" @input="onInputChange" :class="!valid.valid ? 'valid_error' : ''" name="input"
            type="text" />
        </view>
        <!-- 字段按钮组 -->
        <block v-if="fieldData.buttons && fieldData.buttons.length > 0">
          <view class="grid text-center col-4" style="min-width: 25%;">
            <view class="padding-sm " :class="btn.color ? 'line-' + btn.color : 'line-blue'"
              @click.stop="onButtons(fieldData, btn)" :key="index" v-for="(btn, index) in fieldData.buttons">
    ...
    <view class="cu-modal" :class="modalName == 'Image' ? 'show' : ''" @tap="hideModal">
      <view class="cu-dialog" style="height: 100vh;width:100vw;z-index: 999999;">
        <view class="bg-img"
          :style="'background-image: url(' + imagesUrl[imageIndex] + ');height:100%;background-size:100%;'">
    ...
    <view class="cu-modal bottom-modal" :class="{ show: showTreeSelector }">
      <view class="cu-dialog tree-selector">
        <u-search :show-action="false" action-text="搜索" :animation="true" :shape="'square'" v-model="treeSearchVal"
          @change="getTreeSelectorDataWithKey" style="margin: 20rpx;"></u-search>
        <bxTreeSelector :srvInfo="isArray(fieldData.option_list_v2) ? null : fieldData.option_list_v2"
          ...
          @multi-change="multiChange" :defaultData="fieldData.value"
        ...
        <u-loadmore @loadmore="loadMoreTreeData" :status="treeDataStatus" :load-text="loadText"
          v-if="!fieldData.option_list_v2||!fieldData.option_list_v2.is_tree" />
        <view class="dialog-button">
          <view class="cu-btn bg-gray round shadow margin-right" @tap="hideTreeSelector">取消</view>
          <view class="cu-btn bg-blue shadow round" @tap="confirmTreeSelector" v-if="fieldData.isMulti">确定
          </view>
        </view>
      </view>
    </view>
    <view class="cu-modal bottom-modal" :class="{ show: showRichText }" v-if="showRichText">
      <view class="cu-dialog rich-text">
        ...
        <view class="dialog-button">
          ...
          <view class="cu-btn bg-blue shadow" @tap="
							showRichText = false;
							saveEditorValue();
						">
            确定
    ...
    <uni-popup ref="popup" type="bottom" @change="changePopup">
      <cascader-selector @getCascaderValue="getCascaderValue" :srvInfo="fieldData.srvInfo"
        :defaultLineVal="defaultLineVal"></cascader-selector>
    </uni-popup>
  </view>
</template>
...
<script>
  ...
  export default {
    ...
    computed: {
      isDisabled: function() {
        let self = this
        let isDisabled = self.fieldData.disabled
        let colName = self.fieldData.column
        if (colName === self.referencedColName) {
          isDisabled = true
          self.$set(self.fieldData, 'showExp', false)
      ...
      dictShowValue() {
        ...
        let arr = null
        if (self.fieldData.col_type === 'Dict' && option_list_v2.length > 0) {
          let disValue;
          disValue = option_list_v2.filter((item) => item.value === self.fieldData.value)
          // console.log('dictShowValue',disValue,arr,self.fieldData.value,option_list_v2)
          arr = disValue[0];
        } else if (self.fieldData.col_type === 'fk' && self.fieldData.hasOwnProperty('colData')) {
          if (option_list_v2.show_as_pair !== false) {
            arr =
              `${self.fieldData.colData[option_list_v2.refed_col]}/${self.fieldData.colData[option_list_v2.key_disp_col]}`
          } else {
            arr = `${self.fieldData.colData[option_list_v2.key_disp_col]}`
          }
        }
        if (arr && arr.label) {
          return arr.label
      ...
      optionList: function() {
        if (Array.isArray(this.fieldData.options)) {
          if (this.fieldData.disabled && this.fieldData.value && this.pageFormType === 'update') {
            if (['radioFk', 'checkboxFk'].includes(this.fieldData.type)) {
              if (this.fieldData.type === 'radioFk') {
                return this.fieldData.options.filter(item => item.value === this.fieldData.value)
              } else {
                return this.fieldData.options.filter(item => this.fieldData.value.indexOf(item
                  .value) !== -1)
              }
            } else {
              if (this.fieldData.type === 'radio') {
                return this.fieldData.options.filter(item => item === this.fieldData.value)
              } else {
                return this.fieldData.options.filter(item => this.fieldData.value.indexOf(item) !== -1)
              }
            }
          } else {
      ...
      formDataRun: function() {
        ...
        if (this.fieldData.type === 'images') {
          formData['app_no'] = this.appno || uni.getStorageSync('activeApp');
          // this.formData['table_name'] = this.fieldData.srvInfo.tableName;
          formData['columns'] = this.fieldData.column;
          formData['table_name'] = this.fieldData.srvInfo.tableName;
          if (this.fieldData.value !== '' && this.fieldData.value !== null && this.fieldData.value !==
            undefined) {
            formData['file_no'] = this.fieldData.value;
          } else {
            formData['file_no'] = this.GenNonDuplicateID('batchno_')
          }
        } else if (this.fieldData.type === 'file') {
          formData['app_no'] = this.appno || uni.getStorageSync('activeApp');
          // this.formData['table_name'] = this.fieldData.srvInfo.tableName;
          formData['columns'] = this.fieldData.column;
          formData['table_name'] = this.fieldData.srvInfo.tableName;
    ...
    mounted() {
      ...
      if (this.fieldData.type === 'treeSelector' && this.fieldData.display !== false) {
        this.getTreeSelectorData().then(_ => {
          ...
          if (fieldData.value === '[]') {
            this.fieldData.valueForDisplay = ''
            this.treeSelectorShowValue = ''
            this.$emit('on-value-change', this.fieldData);
            return
          }
          if (this.fieldData.col_type === "UserList" && this.fieldData.value && typeof this.fieldData.value ===
            'string') {
            try {
              this.fieldData.value = JSON.parse(this.fieldData.value)
            } catch (e) {
            ...
            if (fieldData.col_type !== 'User' && fieldData.col_type !== 'UserList') {
              if (fieldData.colData && fieldData.value) {
                if (fieldData.option_list_v2.hasOwnProperty('show_as_pair') && fieldData
                  .option_list_v2.show_as_pair !== false) {
                  this.treeSelectorShowValue =
    ...
    created() {
      ...
      if (this.fieldData.type === 'images') {
        ...
        if (this.fieldData.value !== '' && this.fieldData.value !== null && this.fieldData.value !== undefined) {
          ...
          this.formData['file_no'] = this.GenNonDuplicateID('batchno_')
        }
      } else if (this.fieldData.type === 'file') {
        this.formData = {
          serviceName: 'srv_bxfile_service',
    ...
    methods: {
      ...
      async getpoupInfo(info) {
        ...
        if (info.hasOwnProperty('conditions')) {
          for (let i = 0; i < info.conditions.length; i++) {
            let obj = info.conditions[i]
            let val = obj.value
            if (obj.value.indexOf('.')) {
              let key = obj.value.split('.')
              obj.value = self.procData[key[1]]
            }
      ...
      changeVal(newval, oldval, index) {
        ...
        setTimeout(() => {
          if (this.optionsDatasRun[index] && this.optionsDatas[index]) {
            let newValue = this.optionsDatasRun[index]['model'][this.fieldData.optionsConfig['key_col']
              ['value']
            ];
            let oldValue = this.optionsDatas[index]['model'][this.fieldData.optionsConfig['key_col'][
              'value'
            ]];
      ...
      addFileChange(e) {
        console.log('addFileChange', e)
        let fileList = e.allImages
        this.fieldData.value = fileList[0].fileNo || '';
        this.$emit('on-value-change', this.fieldData);
        if (this.fieldData.value !== '' && this.fieldData.value !== null && this.fieldData.value !== undefined) {
          this.formData['file_no'] = this.fieldData.value;
        }
        this.getDefVal()
        setTimeout(() => { // 子组件的值更改后传递到父组件 再从父组件传递到子组件 有延迟 所以加上延时器
          this.onInputChange();
        }, 200)
      },
      deleteFileChange(e) {
        ...
        console.log('deleteFileChange', e)
      },
      getImageInfo(e) {
        ...
        if ((this.fieldData.type === 'images') && this.fieldData.value !== '') {
          ...
          if (fileDatas) {
            for (let i = 0; i < fileDatas.length; i++) {
              ...
              self.imagesUrl.push(self.$api.getFilePath + fileDatas[i].fileurl + '&bx_auth_ticket=' + uni
                .getStorageSync(
                  'bx_auth_ticket'));
            }
          }
          ...
          if (this.fieldData.value) {
            ...
            if (fileDatas) {
              for (let i = 0; i < fileDatas.length; i++) {
                console.log('file:2', self.$api.getFilePath + fileDatas[i].fileurl);
                let obj = {
                  url: self.$api.getFilePath + fileDatas[i].fileurl + '&bx_auth_ticket=' + uni
                    .getStorageSync('bx_auth_ticket'),
                  name: fileDatas[i].src_name,
                  type: fileDatas[i].file_type,
                  appNo: fileDatas[i].app_no,
                  fileNo: fileDatas[i].file_no
                }
                self.imagesUrl.push(obj);
              }
            }
          } else {
            self.imagesUrl = []
          ...
          // console.log('imagesUrl:===>', this.imagesUrl, fileDatas);
        } else if (this.fieldData.type === 'treeSelector' && this.fieldData.disabled == true) {
          // console.log('file:1', this.fieldData, this.field);
          // let url = this.getServiceUrl(app, srv, srvType, url)
          debugger
          this.getTreeSelectorData()
          // console.log('imagesUrl:===>', this.imagesUrl, fileDatas);
        } else {
      ...
      getValid: function() {
        ...
        if (this.fieldData.isRequire && this.fieldData.value) {
          if (this.fieldData.hasOwnProperty('_validators') && this.fieldData._validators.hasOwnProperty(
              'isType') && typeof this
            .fieldData._validators.isType === 'function') {
        ...
        if (self.fieldData._validators && self.fieldData._validators.js_validate) {
          let data = self.fieldsModel
          let result = eval("var fun=" + self.fieldData._validators.js_validate + "(data); fun")
      ...
      onMenu(e) {
        ...
        this.fieldData.value = this.fieldData.option_list_v2 && this.fieldData.option_list_v2['refed_col'] ? data[
          this.fieldData
          .option_list_v2['refed_col']] : data.no;
        ...
        if (fieldData.type === 'treeSelector') {
          ...
          this.fieldData.valueForDisplay = this.treeSelectorShowValue
        }
      },

      hideTreeSelector() {
        this.showTreeSelector = false
        if (this.fieldData.colData && this.fieldData.colData[this.fieldData.option_list_v2.refed_col]) {
          if (this.fieldData.colData[this.fieldData.option_list_v2.refed_col] !== this.fieldData.value) {
            delete this.fieldData.colData
            this.treeSelectorShowValue = this.fieldData.valueForDisplay ? this.fieldData.valueForDisplay : this
              .fieldData.value
          }
        }
      },
      ...
      async getTreeSelectorData(cond, serv, relation_condition) {
        ...
        if (cond) {
          ...
          condition = condition.map(item => {
            if (!item.value) {
              if (item.valueExpr) {
                if (item.valueExpr.lastIndexOf("'") !== -1) {
                  item.value = item.valueExpr.replace(/\'/gi, '')
                }
              }
              return item
            }
        ...
        } else if (self.fieldData.option_list_v2 && self.fieldData.value) {
          // let condition = [{
        ...
        if (req.serviceName === 'srvsso_user_select') {
          // req.condition = [{ colName: 'user_disp', ruleType: 'like', value: 'bx100sys' }];
          req.condition = [{
            colName: 'user_disp',
            ruleType: 'like',
            value: ''
        ...
        debugger
        if (res.data.state === 'SUCCESS' && res.data.page && res.data.page.total > res.data.page.rownumber *
          res.data.page
          .pageNo) {
        ...
        if (res.data.state === 'SUCCESS' && res.data.data.length > 0) {
          ...
          if (hasParentNo && self.pageFormType !== 'detail' & self.fieldData.option_list_v2 && self.fieldData
            .option_list_v2.is_tree) {
            let idCol = 'no'
            if (self.fieldData.option_list_v2 && self.fieldData.option_list_v2.refed_col) {
              idCol = self.fieldData.option_list_v2.refed_col
            }
            self.treeSelectorData = self.treeReform(res.data.data, 'parent_no', idCol);
            if (res.data.page && res.data.page.pageNo > 1) {
              let data = self.treeReform(res.data.data, 'parent_no', idCol);
          ...
          self.treeSelectorData.forEach(item => {
            if (self.fieldData.option_list_v2 && self.fieldData.option_list_v2.refed_col && item[
                self.fieldData.option_list_v2
                .refed_col] === self.fieldData.value) {
              self.fieldData['colData'] = item;
              let fieldData = self.fieldData
              if (fieldData.option_list_v2.hasOwnProperty('show_as_pair') && fieldData
                .option_list_v2.show_as_pair !== false) {
              ...
              self.fieldData.valueForDisplay = self.treeSelectorShowValue
              self.$set(self.fieldData, 'colData', item)
            }
          });
        } else if (req.serviceName === 'srvsys_service_columnex_v2_select' && res.data && res.data.data &&
          Array.isArray(
            res.data.data.srv_cols)) {
...
<style lang="scss" scoped>
  ...
  // 	}
  // }
  .u-radio__icon-wrap--checked.u-radio__icon-wrap--disabled--checked {
    .u-icon__icon.uicon-checkbox-mark {
      color: rgb(100, 150, 197) !important;
    }
  }
