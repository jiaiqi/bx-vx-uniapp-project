<template>
	<view>
		<view class="cu-form-group itemwrap" v-if="fieldData.display"
			...
			<view v-if="pageFormType === 'detail'" class="detail-text" :class="{'row-wrap':fieldData.isMulti}">
				...
				<view class="" v-else-if="pageFormType === 'detail' && fieldData.type === 'images'">
					<image v-if="fieldData.type === 'images'" v-for="(item, index) in imagesUrl" :key="index"
						style="padding: 5upx;" class="cu-avatar radius lg" @tap="showModal(index, 'Image')"
						data-target="Image" :src="item"></image>
				</view>
			</view>
			...
			<view class="form-content" :class="{
				...
				<checkbox-group name="checkbox-group" class="checkbox-group" v-else-if="fieldData.type === 'checkboxFk'"
					:class="!valid.valid ? 'valid_error' : ''">
					<label v-for="(item, index) in optionList" :key="index" class="checkbox"
				...
				<textarea style="min-height: 60px;width: 100%;"
					...
					@blur="onInputBlur" @focus="onInputfocus" auto-height v-model="fieldData.value"
					@input="onInputChange" :disabled="isDisabled ? isDisabled : false"
					:class="!valid.valid ? 'valid_error' : ''" v-else-if="fieldData.type === 'textarea' && showTextarea"
					:placeholder="'输入' + fieldData.label"></textarea>
				<input @click="!isDisabled?showRichText = true:''" type="text"
					v-if="(fieldData.type === 'snote' || fieldData.type === 'Note')"
					:placeholder="'点击编辑' + fieldData.label" :value="html2text(fieldData.value)"
				...
				<view class="content padding-0"
					...
					<input @blur="onInputBlur" @input="onInputChange" v-model="fieldData.value" disabled
						class="date-input" @click.stop="isDisabled ? false : toggleTab(fieldData.type)"
						:placeholder="'请选择' + fieldData.label" :class="!valid.valid ? 'valid_error' : ''"
						style="width: 100%;" name="input" />
					<text class="input-icon cuIcon-calendar"
					...
					<w-picker mode="dateTime" startYear="1900" endYear="2030" step="1" :current="false"
					...
					<view class="cu-list menu card-menu ">
						<view class="cu-item" v-for="(item, index) in optionsDatasRun" :key="index">
							...
							<view class="buttons">
								...
								<view v-if="item.type !== 'draft' && item['model']['option_value']"
				...
				<view class="item-group flex align-center" style="" v-else-if="fieldData.type === 'input'">
					<input @blur="onInputBlur"
						:maxlength="fieldData.item_type_attr && fieldData.item_type_attr.max_len ? fieldData.item_type_attr.max_len : 100"
						:disabled="isDisabled ? isDisabled : false" :placeholder="'输入' + fieldData.label"
						v-model="fieldData.value" @input="onInputChange" :class="!valid.valid ? 'valid_error' : ''"
				...
				<block v-if="fieldData.buttons && fieldData.buttons.length > 0">
					<view class="grid text-center col-4" style="min-width: 25%;">
						<view class="padding-sm " :class="btn.color ? 'line-' + btn.color : 'line-blue'"
							@click.stop="onButtons(fieldData, btn)" :key="index"
		...
		<view class="cu-modal bottom-modal" :class="{ show: showTreeSelector }">
			<view class="cu-dialog tree-selector">
				<u-search :show-action="false" action-text="搜索" :animation="true" :shape="'square'"
					v-model="treeSearchVal" @change="getTreeSelectorDataWithKey" style="margin: 20rpx;"></u-search>
		...
		computed: {
			...
			dictShowValue() {
				...
				if (self.fieldData.col_type === 'Dict' && option_list_v2.length > 0) {
					...
					if (option_list_v2.show_as_pair) {
						arr =
		...
		mounted() {
			...
			if (this.fieldData.type === 'treeSelector' && this.fieldData.display !== false) {
				...
				this.getTreeSelectorData().then(_ => {
					...
					if(this.fieldData.col_type==="UserList"&&this.fieldData.value&&typeof this.fieldData.value==='string'){
						try{
