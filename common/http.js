...
fly.interceptors.request.use((request) => {
  ...
  if (request.url.indexOf('/select/') !== -1) {
    let arr = request.url.split('/select/')
    if (arr.length === 2 && !arr[1]) {
      console.warn(`请求地址${request.url}有误`)
      return Promise.resolve({
        data: {
          state: "FAILURE",
          stateMessage: `请求地址${request.url}有误`
        }
      })
    }
  }
  let ignoreLoadingList = ['srvfile_attachment_select', 'srvdaq_activity_result_save']
  if (!ignoreLoadingList.includes(request.url)) {
    uni.showLoading({
      mask: true
    })
  }
    ...
    if (res.data.resultCode === "0011" || (res.request.headers.USERlOGIN && res.request.headers.USERlOGIN ===
      ...
      if (uni.getStorageSync("isLogin")) {
        ...
        if (res.request.headers.requrl) {
          ...
          if (res.request.headers.requrl && res.request.headers.requrl !== '/' && res.request.headers.requrl.indexOf(
              "code") ===
            -1) {
            //  过滤无效的url
      ...
      let expire_timestamp = parseInt(new Date().getTime() / 1000) + uni.getStorageSync('expire_time'); //过期时间的时间戳(秒)
